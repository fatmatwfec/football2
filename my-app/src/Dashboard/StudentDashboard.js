...
const StudentDashboard = () => {
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-slate-950 via-slate-900 to-black text-white selection:bg-green-500/30 font-['Lexend']">
            {/* Navbar */}
            <nav className="w-full border-b border-white/10 backdrop-blur-lg sticky top-0 z-[100]">
                <div className="max-w-7xl mx-auto flex justify-between items-center px-4 py-3 md:px-6">
                    <div className="flex items-center gap-4 md:gap-8">
                        <div className="md:hidden">
                            ...
                            >
                                {isMobileMenuOpen ? <FaTimes size={24} /> : <div className="space-y-1.5"><div className="w-6 h-0.5 bg-current"></div><div className="w-6 h-0.5 bg-current"></div><div className="w-4 h-0.5 bg-current"></div></div>}
                            </button>
                        </div>
                        <h1
                            className="text-xl md:text-2xl font-black bg-gradient-to-r from-green-400 to-emerald-600 text-transparent bg-clip-text cursor-pointer tracking-tighter"
                        ...
                        <div className="hidden md:flex gap-6">
                            <button
                                onClick={() => setActiveView("dashboard")}
                                className={`text-sm font-bold uppercase transition-all ${activeView === 'dashboard' ? 'text-green-400 border-b-2 border-green-400' : 'text-gray-400 hover:text-white'}`}
                            >
                                Dashboard
                            </button>
                            <button
                                onClick={() => setActiveView("tournament")}
                                className={`text-sm font-bold uppercase transition-all ${activeView === 'tournament' ? 'text-green-400 border-b-2 border-green-400' : 'text-gray-400 hover:text-white'}`}
                            >
                                Tournament
                            </button>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white px-4 py-2 rounded-xl transition font-bold text-xs md:text-sm whitespace-nowrap"
                        >
                            Sign Out
                        </button>
                    </div>
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-full left-0 w-full bg-slate-900/95 backdrop-blur-xl border-b border-white/10 p-4 animate-fade-in z-50">
                        <div className="flex flex-col gap-4">
                            <button
                                onClick={() => { setActiveView("dashboard"); setIsMobileMenuOpen(false); }}
                                className={`text-left p-3 rounded-xl font-bold uppercase text-sm ${activeView === 'dashboard' ? 'bg-green-500/10 text-green-400' : 'text-gray-400'}`}
                            >
                                Dashboard
                            </button>
                            <button
                                onClick={() => { setActiveView("tournament"); setIsMobileMenuOpen(false); }}
                                className={`text-left p-3 rounded-xl font-bold uppercase text-sm ${activeView === 'tournament' ? 'bg-green-500/10 text-green-400' : 'text-gray-400'}`}
                            >
                                Tournament
                            </button>
                        </div>
                ...
                {activeView === "dashboard" ? (
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        <aside className="lg:col-span-4 space-y-6">
                            {/* Profile Card */}
                            <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-white/10 text-center shadow-xl">
                                {/* حاوية الصورة أو الحرف */}
                                <div className="w-24 h-24 md:w-40 md:h-40 mx-auto mb-4 md:mb-6 flex items-center justify-center rounded-full overflow-hidden shadow-lg border-2 border-white/10">
                                    {userData?.photo ? (
                                ...
                                {/* بيانات الطالب */}
                                <h2 className="text-xl md:text-2xl font-bold">{userData?.name || "Student Name"}</h2>
                                <p className="text-gray-400 mt-1 text-sm md:text-base">ID : {userData?.studentCode || "N/A"}</p>
                        ...
                        <main className="lg:col-span-8 space-y-6">
                            ...
                            {tournament?.registrationOpen && (
                                <div className="bg-gradient-to-br from-emerald-600/20 to-transparent backdrop-blur-xl rounded-3xl p-4 md:p-8 border border-emerald-500/20 shadow-xl mb-6 relative overflow-hidden">
                                    <div className="absolute top-0 right-0 p-3 md:p-4 flex flex-col items-end gap-2">
